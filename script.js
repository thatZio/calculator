document.addEventListener('DOMContentLoaded', () => {
    ...
    const calculatePureCash = (inputs) => {
     ...
     const calculateHousePlusCash = (inputs, selectedSize, housingEligibleCompTotal) => {
         ...
         breakdown_part1.push({ name: `住宅区位+旧房(房部分)`, value: p1_comp1 + p1_comp2, formula: `${formatArea(resArea_h)}㎡ approx` });
         breakdown_part1.push({ name: `杂物间区位+旧房(房部分)`, value: p1_comp3_adj + p1_comp4_adj, formula: `${formatArea(storArea_eff_h)}㎡ eff. approx` });
         const p1_comp5 = publicCompArea_h * (rates.locationRate + PUBLIC_AREA_DIFF_HOUSING);
         breakdown_part1.push({ name: "公摊补偿(房)", value: p1_comp5, formula: `${formatArea(publicCompArea_h)}㎡ × (${formatRate(rates.locationRate)} + ${PUBLIC_AREA_DIFF_HOUSING})/㎡` });
         const p1_comp6 = confirmedArea_h * rates.structureRate;
         breakdown_part1.push({ name: "房屋结构等级优惠补差款(房)", value: p1_comp6, formula: `${formatArea(confirmedArea_h)}㎡ × ${formatRate(rates.structureRate)}/㎡` });
         const p1_comp7 = confirmedArea_h * COMPLETE_APT_RATE;
         breakdown_part1.push({ name: "成套房补贴(房)", value: p1_comp7, formula: `${formatArea(confirmedArea_h)}㎡ × ${COMPLETE_APT_RATE}/㎡` });
         const p1_moving = confirmedArea_h * MOVING_RATE * 2;
         breakdown_part1.push({ name: "搬家费(房部分×2)", value: p1_moving, formula: `${formatArea(confirmedArea_h)}㎡ × ${MOVING_RATE}/㎡ × 2次` });
         // Corrected formula display for transition+additional
         const p1_transition_addnl = confirmedArea_h * TRANSITION_RATE * TOTAL_MONTHS_HOUSING;
         breakdown_part1.push({ name: `过渡费+增发(房部分×${TOTAL_MONTHS_HOUSING}月)`, value: p1_transition_addnl, formula: `${formatArea(confirmedArea_h)}㎡ × ${TRANSITION_RATE}/㎡/月 × ${TOTAL_MONTHS_HOUSING}月` });
         const p1_settling = confirmedArea_h * SETTLING_RATE;
         breakdown_part1.push({ name: "安家补贴(房部分)", value: p1_settling, formula: `${formatArea(confirmedArea_h)}㎡ × ${SETTLING_RATE}/㎡` });
         ...
         breakdown_part2.push({ name: "住宅区位+旧房(币部分)", value: p2_comp1 + p2_comp2, formula: `${formatArea(resArea_c)}㎡ approx`});
         breakdown_part2.push({ name: "杂物间区位+旧房(币部分)", value: p2_comp3_adj + p2_comp4_adj, formula: `${formatArea(storArea_eff_c)}㎡ eff. approx`});
         const p2_comp5_cash = publicCompArea_c * (rates.locationRate + MONETARY_REWARD_RATE);
         breakdown_part2.push({ name: "公摊补偿(币)", value: p2_comp5_cash, formula: `${formatArea(publicCompArea_c)}㎡ × (${formatRate(rates.locationRate)} + ${MONETARY_REWARD_RATE})/㎡` });
         const p2_comp6_cash = confirmedArea_c * MONETARY_REWARD_RATE;
         breakdown_part2.push({ name: "货币奖励(币)", value: p2_comp6_cash, formula: `${formatArea(confirmedArea_c)}㎡ × ${MONETARY_REWARD_RATE}/㎡` });
         const p2_comp7_cash = confirmedArea_c * COMPLETE_APT_RATE;
         breakdown_part2.push({ name: "成套房补贴(币)", value: p2_comp7_cash, formula: `${formatArea(confirmedArea_c)}㎡ × ${COMPLETE_APT_RATE}/㎡` });
         const p2_moving = confirmedArea_c * MOVING_RATE * 1;
         breakdown_part2.push({ name: "搬家费(币部分×1)", value: p2_moving, formula: `${formatArea(confirmedArea_c)}㎡ × ${MOVING_RATE}/㎡ × 1次` });
         const p2_transition = confirmedArea_c * TRANSITION_RATE * TRANSITION_MONTHS_CASH;
         breakdown_part2.push({ name: `过渡费(币部分×${TRANSITION_MONTHS_CASH}月)`, value: p2_transition, formula: `${formatArea(confirmedArea_c)}㎡ × ${TRANSITION_RATE}/㎡/月 × ${TRANSITION_MONTHS_CASH}月` });
         const p2_settling = confirmedArea_c * SETTLING_RATE;
         breakdown_part2.push({ name: "安家补贴(币部分)", value: p2_settling, formula: `${formatArea(confirmedArea_c)}㎡ × ${SETTLING_RATE}/㎡` });
