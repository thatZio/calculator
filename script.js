document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeApp() {
        ...
        let currentInputs = {};
        let currentLossDetails = []; // Store loss breakdown for single comparison
        ...
        const calculateHousingEligibleCompAndStructure = (inputs) => { const { resArea, storageInputs, confirmedAreaPrecise, publicCompArea, resBlock, resRates } = inputs; let housingEligibleComp = 0, totalStructureComp = 0; const resBaseValue = resArea * (resRates.locationRate + resRates.oldHouseRate); const resStructureComp = resArea * resRates.structureRate; housingEligibleComp += resBaseValue + resStructureComp; totalStructureComp += resStructureComp; storageInputs.forEach((stor) => { const storRates = getRates(stor.block); const storBaseValue = stor.effectiveArea * (storRates.locationRate + storRates.oldHouseRate); const storStructureComp = stor.effectiveArea * storRates.structureRate; housingEligibleComp += storBaseValue + storStructureComp; totalStructureComp += storStructureComp; }); const publicCompValue = publicCompArea * (resRates.locationRate + PUBLIC_AREA_DIFF_HOUSING); housingEligibleComp += publicCompValue; const completeAptComp = confirmedAreaPrecise * COMPLETE_APT_RATE; housingEligibleComp += completeAptComp; return { housingEligibleComp: round(housingEligibleComp, 2), totalStructureComp: totalStructureComp }; };
        ...
        // --- Original Value Split Logic (Renamed for Block A) ---
        const calculateXHousePlusCash_ValueSplit = (inputs, combo) => { const { resArea, storageInputs, confirmedArea, confirmedAreaPrecise, publicCompArea, resBlock, resRates, isPublicHousing, equivalentArea, resettlementArea, housingEligibleComp, totalStructureComp, relocationRewardTiered, publicHousingDeductionAmount, decorationFee } = inputs; let breakdown_part1 = [], breakdown_part2 = [], totalPart1Value = 0, totalPart2Value = 0; const selectedTotalArea = combo.reduce((sum, size) => sum + size, 0); const houseValue = selectedTotalArea * HOUSING_PRICE; let propHouse = 0; if (housingEligibleComp > 0) { propHouse = Math.min(houseValue / housingEligibleComp, 1); } if (propHouse > 1 && propHouse < 1.00001) propHouse = 1; const propCash = Math.max(0, 1 - propHouse); const resLocOldValue_H = resArea * (resRates.locationRate + resRates.oldHouseRate); const resStructValue_H = resArea * resRates.structureRate; let storLocOldValue_H_Total = 0, storStructValue_H_Total = 0; storageInputs.forEach(stor => { const storRates = getRates(stor.block); storLocOldValue_H_Total += stor.effectiveArea * (storRates.locationRate + storRates.oldHouseRate); storStructValue_H_Total += stor.effectiveArea * storRates.structureRate; }); const publicCompValue_H = publicCompArea * (resRates.locationRate + PUBLIC_AREA_DIFF_HOUSING); const completeAptValue_Total = confirmedAreaPrecise * COMPLETE_APT_RATE; const structureBonusValue_Total = totalStructureComp; const movingFee2Total = Math.max(confirmedArea * MOVING_RATE_BASE * 2, MIN_MOVING_2); const transitionHousingTotal = confirmedAreaPrecise * TRANSITION_RATE * TRANSITION_MONTHS_HOUSING; const settlingTotal = confirmedAreaPrecise * SETTLING_RATE; const movingFee1Total = Math.max(confirmedArea * MOVING_RATE_BASE * 1, MIN_MOVING_1); const transitionCashTotal = confirmedAreaPrecise * TRANSITION_RATE * TRANSITION_MONTHS_CASH; const publicCompValue_C = publicCompArea * (resRates.locationRate + PUBLIC_AREA_DIFF_CASH); const p1_resLocOld = resLocOldValue_H * propHouse; breakdown_part1.push({ name: `住宅区位+旧房(房部分 ${resBlock})`, value: p1_resLocOld, formula: `住宅价值按比例 ${formatArea(propHouse*100)}%`}); totalPart1Value += p1_resLocOld; const p1_storLocOld = storLocOldValue_H_Total * propHouse; breakdown_part1.push({ name: `杂物间区位+旧房(房部分 合计)`, value: p1_storLocOld, formula: `杂物间价值按比例 ${formatArea(propHouse*100)}%` }); totalPart1Value += p1_storLocOld; const p1_publicComp = publicCompValue_H * propHouse; breakdown_part1.push({ name: "公摊补偿(房部分)", value: p1_publicComp, formula: `公摊价值按比例 ${formatArea(propHouse*100)}%` }); totalPart1Value += p1_publicComp; const p1_structureComp = structureBonusValue_Total * propHouse; breakdown_part1.push({ name: "房屋结构等级优惠(房部分)", value: p1_structureComp, formula: `结构总款按比例 ${formatArea(propHouse*100)}%` }); totalPart1Value += p1_structureComp; const p1_completeAptComp = completeAptValue_Total * propHouse; breakdown_part1.push({ name: "成套房补贴(房部分)", value: p1_completeAptComp, formula: `成套房总补贴按比例 ${formatArea(propHouse*100)}%` }); totalPart1Value += p1_completeAptComp; const p1_moving = movingFee2Total * propHouse; breakdown_part1.push({ name: "搬家费(房部分)", value: p1_moving, formula: `2次总额 ${formatMoney(movingFee2Total)} × ${formatArea(propHouse * 100)}%` }); totalPart1Value += p1_moving; const p1_transition = transitionHousingTotal * propHouse; breakdown_part1.push({ name: `过渡费+增发(房部分×${TRANSITION_MONTHS_HOUSING}月)`, value: p1_transition, formula: `房部分比例 ${formatArea(propHouse*100)}%` }); totalPart1Value += p1_transition; const p1_settling = settlingTotal * propHouse; breakdown_part1.push({ name: "安家补贴(房部分)", value: p1_settling, formula: `房部分比例 ${formatArea(propHouse*100)}%` }); totalPart1Value += p1_settling; const p2_resLocOld = resLocOldValue_H * propCash; breakdown_part2.push({ name: `住宅区位+旧房(币部分 ${resBlock})`, value: p2_resLocOld, formula: `住宅价值按比例 ${formatArea(propCash*100)}%`}); totalPart2Value += p2_resLocOld; const p2_storLocOld = storLocOldValue_H_Total * propCash; breakdown_part2.push({ name: `杂物间区位+旧房(币部分 合计)`, value: p2_storLocOld, formula: `杂物间价值按比例 ${formatArea(propCash*100)}%` }); totalPart2Value += p2_storLocOld; const p2_publicComp = publicCompValue_C * propCash; breakdown_part2.push({ name: "公摊补偿(币部分)", value: p2_publicComp, formula: `公摊货币价值按比例 ${formatArea(propCash*100)}%` }); totalPart2Value += p2_publicComp; const confirmedAreaPrecise_c = confirmedAreaPrecise * propCash; const p2_monetaryReward = confirmedAreaPrecise_c * MONETARY_REWARD_RATE; breakdown_part2.push({ name: "货币奖励(币部分)", value: p2_monetaryReward, formula: `${formatArea(confirmedAreaPrecise_c)}㎡ × ${MONETARY_REWARD_RATE}/㎡` }); totalPart2Value += p2_monetaryReward; const p2_completeAptComp = completeAptValue_Total * propCash; breakdown_part2.push({ name: "成套房补贴(币部分)", value: p2_completeAptComp, formula: `成套房总补贴按比例 ${formatArea(propCash*100)}%` }); totalPart2Value += p2_completeAptComp; const p2_moving = movingFee1Total * propCash; breakdown_part2.push({ name: "搬家费(币部分)", value: p2_moving, formula: `1次总额 ${formatMoney(movingFee1Total)} × ${formatArea(propCash * 100)}%` }); totalPart2Value += p2_moving; const p2_transition = transitionCashTotal * propCash; breakdown_part2.push({ name: `过渡费(币部分×${TRANSITION_MONTHS_CASH}月)`, value: p2_transition, formula: `币部分比例 ${formatArea(propCash*100)}%` }); totalPart2Value += p2_transition; const p2_settling = settlingTotal * propCash; breakdown_part2.push({ name: "安家补贴(币部分)", value: p2_settling, formula: `币部分比例 ${formatArea(propCash*100)}%` }); totalPart2Value += p2_settling; breakdown_part2.push({ name: "搬迁奖励", value: relocationRewardTiered, formula: `按确权面积 ${formatArea(confirmedArea)}㎡ 档` }); totalPart2Value += relocationRewardTiered; breakdown_part2.push({ name: "租房补贴", value: RENTAL_SUBSIDY, formula: `固定 ${formatMoney(RENTAL_SUBSIDY)}` }); totalPart2Value += RENTAL_SUBSIDY; let fullBreakdown = [...breakdown_part1, ...breakdown_part2]; let combinedValue = totalPart1Value + totalPart2Value; if (decorationFee > 0) { fullBreakdown.push({ name: "装修评估费", value: decorationFee, formula: "评估确定" }); combinedValue += decorationFee; } if (isPublicHousing && publicHousingDeductionAmount !== 0) { fullBreakdown.push({ name: "公房扣减", value: publicHousingDeductionAmount, formula: `${formatArea(confirmedAreaPrecise)}㎡ × ${formatRate(resRates.locationRate)}/㎡ × ${PUBLIC_HOUSING_FACTOR * 100}%`, isDeduction: true }); combinedValue += publicHousingDeductionAmount; } const totalCompensation = round(combinedValue, 0); const finalDifference = totalCompensation - round(houseValue, 0); const scenarioType = combo.length === 1 ? "1 House + Cash" : "2 Houses + Cash"; const scenarioName = `${combo.sort((a, b) => b - a).join('㎡ + ')}㎡ + 货币`; const scenarioId = `xhc_${combo.sort((a, b) => a - b).join('_')}_${Date.now()}`; return { id: scenarioId, type: scenarioType, name: scenarioName, selectedArea: selectedTotalArea, combo: combo.sort((a, b) => b - a), totalCompensation: totalCompensation, housingCost: round(houseValue, 0), finalDifference: finalDifference, breakdown: fullBreakdown, housingEligibleComp: housingEligibleComp, equivalentArea: equivalentArea, resettlementArea: resettlementArea, confirmedArea: confirmedArea, publicCompArea: publicCompArea, isPublicHousing: isPublicHousing, publicHousingDeductionAmount: publicHousingDeductionAmount, relocationReward: relocationRewardTiered, propHouse: propHouse, propCash: propCash, totalStructureComp: totalStructureComp, decorationFee: decorationFee }; };
        // --- NEW Area Split Logic (for Blocks B, C, D) ---
        const calculateXHousePlusCash_AreaSplit = (inputs, combo) => { const { resArea, storageInputs, confirmedArea, confirmedAreaPrecise, publicCompArea, resBlock, resRates, isPublicHousing, equivalentArea, resettlementArea, /* HEC not used */ /* totalStructureComp needs careful handling */ relocationRewardTiered, publicHousingDeductionAmount, decorationFee } = inputs; const selectedTotalArea = combo.reduce((sum, size) => sum + size, 0); const houseValue = round(selectedTotalArea * HOUSING_PRICE, 0); let breakdown_part1 = [], breakdown_part2 = [], totalPart1Comp = 0, totalPart2Comp = 0; let resArea_P1 = round(selectedTotalArea / 1.1, 2); resArea_P1 = Math.min(resArea_P1, resArea); const resArea_P2 = round(resArea - resArea_P1, 2); const publicCompArea_P1 = round(Math.min(resArea_P1 * 0.1, MAX_PUBLIC_AREA), 2); const publicCompArea_P2 = round(publicCompArea - publicCompArea_P1, 2); const storageEffectiveArea_P2 = storageInputs.reduce((sum, stor) => sum + stor.effectiveArea, 0); const effectiveArea_P2 = round(resArea_P2 + storageEffectiveArea_P2, 2); const comp1_loc_old = resArea_P1 * (resRates.locationRate + resRates.oldHouseRate); breakdown_part1.push({ name: `住宅区位+旧房(房部分 ${resBlock})`, value: comp1_loc_old, formula: `${formatArea(resArea_P1)}㎡ × (${formatRate(resRates.locationRate)} + ${formatRate(resRates.oldHouseRate)})/㎡` }); totalPart1Comp += comp1_loc_old; const comp1_pub = publicCompArea_P1 * (resRates.locationRate + PUBLIC_AREA_DIFF_HOUSING); breakdown_part1.push({ name: "公摊补偿(房部分)", value: comp1_pub, formula: `${formatArea(publicCompArea_P1)}㎡ × (${formatRate(resRates.locationRate)} + ${PUBLIC_AREA_DIFF_HOUSING})/㎡` }); totalPart1Comp += comp1_pub; const comp1_struct = resArea_P1 * resRates.structureRate; breakdown_part1.push({ name: `房屋结构等级优惠(房部分)`, value: comp1_struct, formula: `${formatArea(resArea_P1)}㎡ × ${formatRate(resRates.structureRate)}/㎡` }); totalPart1Comp += comp1_struct; const comp1_compapt = resArea_P1 * COMPLETE_APT_RATE; breakdown_part1.push({ name: "成套房补贴(房部分)", value: comp1_compapt, formula: `${formatArea(resArea_P1)}㎡ × ${COMPLETE_APT_RATE}/㎡` }); totalPart1Comp += comp1_compapt; const movingFee_P1_calc = resArea_P1 * MOVING_RATE_BASE * 2; const movingFee_P1 = Math.max(movingFee_P1_calc, MIN_MOVING_2); breakdown_part1.push({ name: "搬家费(房部分)", value: movingFee_P1, formula: `MAX(${formatArea(resArea_P1)}㎡ × ${MOVING_RATE_BASE}/㎡ × 2, ${formatMoney(MIN_MOVING_2)})` }); totalPart1Comp += movingFee_P1; const transition_P1 = resArea_P1 * TRANSITION_RATE * TRANSITION_MONTHS_HOUSING; breakdown_part1.push({ name: `过渡费+增发(房部分×${TRANSITION_MONTHS_HOUSING}月)`, value: transition_P1, formula: `${formatArea(resArea_P1)}㎡ × ${TRANSITION_RATE}/㎡/月 × ${TRANSITION_MONTHS_HOUSING}月` }); totalPart1Comp += transition_P1; const settling_P1 = resArea_P1 * SETTLING_RATE; breakdown_part1.push({ name: "安家补贴(房部分)", value: settling_P1, formula: `${formatArea(resArea_P1)}㎡ × ${SETTLING_RATE}/㎡` }); totalPart1Comp += settling_P1; const comp2_res_loc_old = resArea_P2 * (resRates.locationRate + resRates.oldHouseRate); breakdown_part2.push({ name: `住宅区位+旧房(币部分 ${resBlock})`, value: comp2_res_loc_old, formula: `${formatArea(resArea_P2)}㎡ × (${formatRate(resRates.locationRate)} + ${formatRate(resRates.oldHouseRate)})/㎡` }); totalPart2Comp += comp2_res_loc_old; let comp2_stor_loc_old_total = 0; storageInputs.forEach((stor, index) => { const storRates = getRates(stor.block); const storComp = stor.effectiveArea * (storRates.locationRate + storRates.oldHouseRate); breakdown_part2.push({ name: `杂物间${index + 1}区位+旧房 (${stor.block}地块)`, value: storComp, formula: `${formatArea(stor.effectiveArea)}㎡ × (${formatRate(storRates.locationRate)} + ${formatRate(storRates.oldHouseRate)})/㎡` }); comp2_stor_loc_old_total += storComp; }); totalPart2Comp += comp2_stor_loc_old_total; const comp2_pub = publicCompArea_P2 * (resRates.locationRate + PUBLIC_AREA_DIFF_CASH); breakdown_part2.push({ name: "公摊补偿(币部分)", value: comp2_pub, formula: `${formatArea(publicCompArea_P2)}㎡ × (${formatRate(resRates.locationRate)} + ${PUBLIC_AREA_DIFF_CASH})/㎡` }); totalPart2Comp += comp2_pub; const comp2_monetary = effectiveArea_P2 * MONETARY_REWARD_RATE; breakdown_part2.push({ name: "货币奖励(币部分)", value: comp2_monetary, formula: `${formatArea(effectiveArea_P2)}㎡ × ${MONETARY_REWARD_RATE}/㎡` }); totalPart2Comp += comp2_monetary; const comp2_compapt = effectiveArea_P2 * COMPLETE_APT_RATE; breakdown_part2.push({ name: "成套房补贴(币部分)", value: comp2_compapt, formula: `${formatArea(effectiveArea_P2)}㎡ × ${COMPLETE_APT_RATE}/㎡` }); totalPart2Comp += comp2_compapt; const movingFee_P2_calc = effectiveArea_P2 * MOVING_RATE_BASE * 1; let finalMovingFee_P2 = movingFee_P2_calc; if (movingFee_P1 === MIN_MOVING_2 && movingFee_P1_calc < MIN_MOVING_2) { console.log("Applying tentative rule: Moving Fee P2 set to 0 because P1 hit minimum."); finalMovingFee_P2 = 0; } if (finalMovingFee_P2 > 0) { breakdown_part2.push({ name: "搬家费(币部分)", value: finalMovingFee_P2, formula: `${formatArea(effectiveArea_P2)}㎡ × ${MOVING_RATE_BASE}/㎡ × 1 (无最低标准?)` }); totalPart2Comp += finalMovingFee_P2; } const transition_P2 = effectiveArea_P2 * TRANSITION_RATE * TRANSITION_MONTHS_CASH; breakdown_part2.push({ name: `过渡费(币部分×${TRANSITION_MONTHS_CASH}月)`, value: transition_P2, formula: `${formatArea(effectiveArea_P2)}㎡ × ${TRANSITION_RATE}/㎡/月 × ${TRANSITION_MONTHS_CASH}月` }); totalPart2Comp += transition_P2; const settling_P2 = effectiveArea_P2 * SETTLING_RATE; breakdown_part2.push({ name: "安家补贴(币部分)", value: settling_P2, formula: `${formatArea(effectiveArea_P2)}㎡ × ${SETTLING_RATE}/㎡` }); totalPart2Comp += settling_P2; breakdown_part2.push({ name: "搬迁奖励", value: relocationRewardTiered, formula: `按确权面积 ${formatArea(confirmedArea)}㎡ 档` }); totalPart2Comp += relocationRewardTiered; breakdown_part2.push({ name: "租房补贴", value: RENTAL_SUBSIDY, formula: `固定 ${formatMoney(RENTAL_SUBSIDY)}` }); totalPart2Comp += RENTAL_SUBSIDY; let fullBreakdown = [...breakdown_part1, ...breakdown_part2]; let combinedValue = totalPart1Comp + totalPart2Comp; if (decorationFee > 0) { fullBreakdown.push({ name: "装修评估费", value: decorationFee, formula: "评估确定" }); combinedValue += decorationFee; } if (isPublicHousing && publicHousingDeductionAmount !== 0) { breakdown_part2.push({ name: "公房扣减", value: publicHousingDeductionAmount, formula: `${formatArea(confirmedAreaPrecise)}㎡ × ${formatRate(resRates.locationRate)}/㎡ × ${PUBLIC_HOUSING_FACTOR * 100}%`, isDeduction: true }); combinedValue += publicHousingDeductionAmount; } const totalCompensation = round(combinedValue, 0); const finalDifference = totalCompensation - houseValue; const scenarioType = combo.length === 1 ? "1 House + Cash" : "2 Houses + Cash"; const scenarioName = `${combo.sort((a, b) => b - a).join('㎡ + ')}㎡ + 货币`; const scenarioId = `xhc_${combo.sort((a, b) => a - b).join('_')}_${Date.now()}`; return { id: scenarioId, type: scenarioType, name: scenarioName, selectedArea: selectedTotalArea, combo: combo.sort((a, b) => b - a), totalCompensation: totalCompensation, housingCost: houseValue, finalDifference: finalDifference, breakdown: fullBreakdown, housingEligibleComp: inputs.housingEligibleComp, equivalentArea: equivalentArea, resettlementArea: resettlementArea, confirmedArea: confirmedArea, publicCompArea: publicCompArea, isPublicHousing: isPublicHousing, publicHousingDeductionAmount: publicHousingDeductionAmount, relocationReward: relocationRewardTiered, propHouse: null, propCash: null, totalStructureComp: inputs.totalStructureComp, decorationFee: decorationFee }; };
        ...
        // --- NEW: Calculate Loss Scenario ---
        const calculateLossScenario = (originalScenario, inputs) => {
             // Create a deep copy of inputs to avoid modifying the original
             const lossInputs = JSON.parse(JSON.stringify(inputs));
             let lostItems = []; // To store details of lost compensation

             // 1. Identify and Zero Out Lost Compensation Items
             let structureLoss = 0;
             let publicCompLoss = 0;
             let completeAptLoss = 0;
             let rentalLoss = RENTAL_SUBSIDY; // Fixed loss
             let rewardLoss = originalScenario.relocationReward || 0; // Use the reward from the original scenario calculation

             // Find these items in the *original* scenario's breakdown to get their value
             originalScenario.breakdown.forEach(item => {
                 if (item.name.includes('结构等级优惠') || item.name.includes('货币奖励')) { // Covers structure and monetary reward
                     structureLoss += item.value;
                 } else if (item.name.includes('公摊补偿')) {
                     publicCompLoss += item.value;
                 } else if (item.name.includes('成套房补贴')) {
                     completeAptLoss += item.value;
                 }
             });

             // Store details for loss breakdown display
             ...
             // 2. Recalculate Scenario WITHOUT lost items
             // We need to simulate the original calculation type but exclude the lost items value.
             // This is complex because the original calculation functions add these items.
             // Easier approach: Start with original total and subtract lost values.

             let lossTotalCompensation = originalScenario.totalCompensation - structureLoss - publicCompLoss - completeAptLoss - rentalLoss - rewardLoss;
             lossTotalCompensation = round(lossTotalCompensation, 0); // Round final value

             // 3. Check if the original housing choice is still affordable
             let canAffordHousing = true;
             let lossHousingEligibleComp = 0; // Need to recalculate HEC without lost items affecting it

             if (originalScenario.type !== "Pure Monetary") {
                   // Recalculate HEC specifically excluding structure/comp apt/public comp housing rate part
                    const { resArea, storageInputs, publicCompArea: origPublicCompArea, resBlock: origResBlock, resRates: origResRates } = lossInputs;
                    let tempHEC = 0;
                    // Add Res Loc + Old
                    ...
                    storageInputs.forEach((stor) => {
                        ...
                        tempHEC += stor.effectiveArea * (storRates.locationRate + storRates.oldHouseRate);
                    });
                    // DO NOT ADD: Structure, Public Comp (Housing Rate), Complete Apt

                    lossHousingEligibleComp = round(tempHEC, 2);

                    // Now check affordability based on this reduced HEC
                    if (lossHousingEligibleComp < originalScenario.housingCost - 1) { // Allow small tolerance
                         canAffordHousing = false;
                    }
             }

             // 4. Calculate final difference for the loss scenario
             let lossFinalDifference = lossTotalCompensation; // Default for pure monetary
             if (originalScenario.type !== "Pure Monetary" && canAffordHousing) {
             ...
             return {
                 ...
                 housingCost: canAffordHousing ? originalScenario.housingCost : 0, // Show 0 cost if cannot afford
                 finalDifference: lossFinalDifference,
                 canAffordHousing: canAffordHousing,
                 lostItems: lostItems // Store the detailed lost items
             };
        };

        // --- NEW: Display Single Comparison ---
        const displaySingleComparison = (originalScenario, lossScenario) => {
             ...
             const metrics = [
                 { key: 'combo', label: '选择房型' }, // No formatter needed here, handled below
                 // { key: 'housingCost', label: '购房款 (元)', formatter: formatMoney }, // Removed
                 { key: 'totalCompensation', label: '补偿款总计 (元)', formatter: formatMoney },
                 { key: 'finalDifference', label: '应交(-)/退(+)差价 (元)' }, // Formatter applied conditionally below
             ];
             ...
             metrics.forEach(metric => {
                 ...
                 let lossValue = lossScenario[metric.key];
                 let formattedOriginalValue = 'N/A';
                 let formattedLossValue = 'N/A';

                 // Handle specific formatting or text changes
                 if (metric.key === 'combo') {
                      formattedOriginalValue = originalScenario.combo && originalScenario.combo.length > 0 ? originalScenario.combo.join('㎡ + ') + '㎡' : '无';
                      if (lossScenario.type !== 'Pure Monetary' && !lossScenario.canAffordHousing) {
                          formattedLossValue = `<span style="color:red; font-style:italic;">补偿面积不足，无法选择此房型</span>`;
                      } else {
                          formattedLossValue = formattedOriginalValue; // Same housing choice if affordable
                      }
                      ...
                      formattedOriginalValue = `<span style="color:${diffColorOrig}; font-weight: bold;">${formatMoney(diffValueOrig)}</span>`;
                       // Format Loss Difference
                       if (lossScenario.type !== 'Pure Monetary' && !lossScenario.canAffordHousing) {
                            formattedLossValue = 'N/A'; // No diff if cannot buy house
                       } else {
                           const diffValueLoss = lossScenario.finalDifference;
                      ...
                      formattedLossValue = metric.formatter(lossValue);
                 } else {
                      formattedOriginalValue = (originalValue === undefined || originalValue === null) ? 'N/A' : originalValue;
                      formattedLossValue = (lossValue === undefined || lossValue === null) ? 'N/A' : lossValue;
                 }
             ...
             // Calculate and display total loss, unless housing not affordable
             if (lossScenario.type === 'Pure Monetary' || lossScenario.canAffordHousing) {
                 const totalLoss = originalScenario.totalCompensation - lossScenario.totalCompensation;
                 comparisonLossSummary.innerHTML = `超过签约期配合征迁手续，总补偿款损失 <span>${formatMoney(totalLoss)}</span> 元。`;
                 comparisonLossSummary.classList.remove('hidden');
                 showLossDetailsButton.classList.remove('hidden'); // Show details button
                 currentLossDetails = lossScenario.lostItems; // Store details
             } else {
                  comparisonLossSummary.classList.add('hidden'); // Hide summary if cannot afford
                  showLossDetailsButton.classList.add('hidden'); // Hide details button
                  currentLossDetails = [];
             }
             comparisonLossDetails.classList.add('hidden'); // Ensure details are hidden initially

        };

         // --- NEW: Display Multi Comparison ---
         const displayMultiComparison = (scenariosToCompare) => {
             let tableHtml = '<table><thead><tr><th>指标</th>';
             scenariosToCompare.forEach(s => { tableHtml += `<th>${s.name}</th>`; });
             tableHtml += '</tr></thead><tbody>';

             // Define metrics for multi-comparison (Removed 'housingCost')
             const metrics = [
        ...
        // Modified to call the new displayComparison orchestrator
